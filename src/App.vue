...
export default {
  ...
  methods:{
    async loginIn(){
      console.log('model => ',this.mail,this.pass);
      const form = new FormData();
      form.append('mail', this.mail);
      form.append('password',this.pass);
      const {data} = await axios('http://localhost:1989/login',{
        method:'post',
        data: form
      });
      console.log(data);
    ...
    async load(){
      /*const form = new FormData();
      form.append('id',window.localStorage.getItem('userID'))*/
      var id = window.localStorage.getItem('userID');
      var token = window.localStorage.getItem('token');

      const {data} = await axios("http://localhost:1989/auth/user/" + id ,{
        method:'get',
